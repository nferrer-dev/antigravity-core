        ...
        (() => {
            ...
            for (const el of articles) {
                ...
                if (fiberKey && el[fiberKey]) {
                    // Get memoizedProps of the actual element or its direct return
                    memoProps = el[fiberKey].memoizedProps;
                    
                    // We only want to pull out potential IDs, like id, className, etc.
                    // to avoid circular JSON
                    if (memoProps) {
                        memoProps = {
                            id: memoProps.id,
                            className: memoProps.className,
                            'data-id': memoProps['data-id'],
                            keys: Object.keys(memoProps)
                        };
                    }
                }
